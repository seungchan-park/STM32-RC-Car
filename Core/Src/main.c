...
osMutexId BTdataMutexHandle;
osMutexId MotorStateMutexHandle;
/* USER CODE BEGIN PV */
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	if (huart->Instance == USART2) {
		Listener_BT_ISR_Process(uart1RxData);
	}
	HAL_UART_Receive_IT(&huart2, &uart1RxData, 1);
}

void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	Listener_UlraSonic_ISR_Process(GPIO_Pin);
}
/* USER CODE END 0 */
...
int main(void)
{
  ...
  /* Create the mutex(es) */
  /* definition and creation of BTdataMutex */
  osMutexDef(BTdataMutex);
  BTdataMutexHandle = osMutexCreate(osMutex(BTdataMutex));

  /* definition and creation of MotorStateMutex */
  osMutexDef(MotorStateMutex);
  MotorStateMutexHandle = osMutexCreate(osMutex(MotorStateMutex));
...
void ListenerTask(void const * argument)
{
  /* USER CODE BEGIN ListenerTask */
	Listener_init(&htim1, &htim2, &htim4);
  /* Infinite loop */
  for(;;)
  {
	  if(osMutexWait(BTdataMutexHandle, osWaitForever) == osOK){
		  Listener_eventCheck();
		  Listener_excuteTask();
		  osMutexRelease(BTdataMutexHandle);
	  }
	  osDelay(1);
  }
...
void ControllerTask(void const * argument)
{
  ...
  {
	  if(osMutexWait(BTdataMutexHandle, osWaitForever) == osOK){
		  if(osMutexWait(MotorStateMutexHandle, osWaitForever) == osOK){
...
void PresenterTask(void const * argument)
{
  /* USER CODE BEGIN PresenterTask */
	Presenter_init(&hi2c1, &htim3);
  /* Infinite loop */
  for(;;)
  {
	  if(osMutexWait(MotorStateMutexHandle, osWaitForever) == osOK){
		  Presenter_excuteTack();
		  osMutexRelease(MotorStateMutexHandle);
	  }
	  osDelay(1);
  }
